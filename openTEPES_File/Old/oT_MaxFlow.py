...
pMaxTheta     = pd.DataFrame(pd.concat([pMaxThetaDiff.loc[mTEPES.rf]]*len(pDemand.index))).reset_index(drop=True).set_index(pDemand.index)
...
def eMaxThetaDiff(mTEPES,sc,p,n,ni,nf):
    if ni > nf and nf != pReferenceNode:
        return (-pMaxThetaDiff.loc[ni,nf], mTEPES.vTheta[sc,p,n,ni] - mTEPES.vTheta[sc,p,n,nf], pMaxThetaDiff.loc[ni,nf])
    else:
