...
# OutputResults = pd.Series(data=[abs(mTEPES.vP[sc,p,n,ni,nf,cc]()/mTEPES.pLineNTC[ni,nf,cc]) for sc,p,n,ni,nf,cc in mTEPES.sc*mTEPES.p*mTEPES.n*mTEPES.la], index= pd.MultiIndex.from_tuples(list(mTEPES.sc*mTEPES.p*mTEPES.n*mTEPES.la)))
# OutputResults.index.names = ['Scenario','Period','LoadLevel','InitialNode','FinalNode','Circuit']
# OutputResults = pd.pivot_table(OutputResults.to_frame(name='pu'), values='pu', index=['Scenario','Period','LoadLevel'], columns=['InitialNode','FinalNode','Circuit'], fill_value=0)
# OutputResults.index.names = [None] * len(OutputResults.index.names)
# OutputResults.to_csv('oT_Result_ActiveNetworkUtilization_'+CaseName+'.csv', sep=',')

# OutputResults = pd.Series(data=[abs(mTEPES.vQ[sc,p,n,ni,nf,cc]()/mTEPES.pLineNTC[ni,nf,cc]) for sc,p,n,ni,nf,cc in mTEPES.sc*mTEPES.p*mTEPES.n*mTEPES.la], index= pd.MultiIndex.from_tuples(list(mTEPES.sc*mTEPES.p*mTEPES.n*mTEPES.la)))
# OutputResults.index.names = ['Scenario','Period','LoadLevel','InitialNode','FinalNode','Circuit']
# OutputResults = pd.pivot_table(OutputResults.to_frame(name='pu'), values='pu', index=['Scenario','Period','LoadLevel'], columns=['InitialNode','FinalNode','Circuit'], fill_value=0)
# OutputResults.index.names = [None] * len(OutputResults.index.names)
# OutputResults.to_csv('oT_Result_ReactiveNetworkUtilization_'+CaseName+'.csv', sep=',')
...
# OutputResults = pd.DataFrame.from_dict(mTEPES.vVoltageMag_sqr.extract_values(), orient='index', columns=[str(mTEPES.vTheta)])
# OutputResults = OutputResults**0.5
# OutputResults.index = pd.MultiIndex.from_tuples(OutputResults.index)
# OutputResults.index.names = ['Scenario','Period','LoadLevel','Node']
# pd.pivot_table(OutputResults, values=str(mTEPES.vVoltageMag_sqr), index=['Scenario','Period','LoadLevel'], columns=['Node'], fill_value=0).to_csv('oT_Result_NetworkVoltage_'+CaseName+'.csv', sep=',')
...
OutputResults = pd.Series(data=[mTEPES.vENS[sc,p,n,nd]() for sc,p,n,nd in mTEPES.sc*mTEPES.p*mTEPES.n*mTEPES.nd], index=pd.MultiIndex.from_tuples(list(mTEPES.sc*mTEPES.p*mTEPES.n*mTEPES.nd)))
OutputResults.to_frame(name='GWh').reset_index().pivot_table(index=['level_0','level_1','level_2'], columns='level_3', values='GWh').rename_axis(['Scenario','Period','LoadLevel'], axis=0).rename_axis([None], axis=1).to_csv('oT_Result_NetworkENS_'+CaseName+'.csv', sep=',')
